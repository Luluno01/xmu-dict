          ...
          if(res.text.match('Fail')) { // Login failed
            ...
            User.findOne({username: option.username}).exec((err, user) => {
              if(err) {
                ...
                User.create({username: option.username, grade: undefined}).exec((err, user) => {
                  _user = user;
                  automata.transit('main');
                });
              } else {
                ...
                if(new Date() - user.updatedAt > config.grade.cacheTTL * 1000 || !(user.grade)) { // Cache expired or empty data
                  sails.log.info('[ Grade ] Cache expired or invalid.');
                  automata.transit('main');
                } else {
